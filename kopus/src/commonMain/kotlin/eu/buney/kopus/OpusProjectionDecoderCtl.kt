...
/**
 * Extension functions for OpusProjectionDecoder to provide easy access to decoder control parameters.
 *
 * These functions wrap calls to the OpusProjectionDecoder.ctl and OpusProjectionDecoder.ctlQuery methods,
 * which internally use the `opus_projection_decoder_ctl` C function.
 */
 ...
 */
fun OpusProjectionDecoder.setGain(gain: Int): Int = ctl(OPUS_SET_GAIN_REQUEST, gain)

/**
 * Gets the decoder's configured gain adjustment.
 * @return Amount to scale PCM signal by in Q8 dB units.
 */
fun OpusProjectionDecoder.getGain(): Int = ctlQuery(OPUS_GET_GAIN_REQUEST)
 ...
 * @return [OPUS_OK] on success
 */
fun OpusProjectionDecoder.setPhaseInversionDisabled(disabled: Boolean): Int = ctl(OPUS_SET_PHASE_INVERSION_DISABLED_REQUEST, if (disabled) 1 else 0)
 ...
 */
fun OpusProjectionDecoder.getPhaseInversionDisabled(): Boolean = ctlQuery(OPUS_GET_PHASE_INVERSION_DISABLED_REQUEST) == 1

/**
 * Gets the duration (in samples) of the last packet successfully decoded or concealed.
 * @return Number of samples (at current sampling rate)
 */
fun OpusProjectionDecoder.getLastPacketDuration(): Int = ctlQuery(OPUS_GET_LAST_PACKET_DURATION_REQUEST)
 ...
 */
fun OpusProjectionDecoder.getPitch(): Int = ctlQuery(OPUS_GET_PITCH_REQUEST)

/**
 * Gets the sampling rate the decoder was initialized with.
 * @return Sampling rate of decoder in Hz
 */
fun OpusProjectionDecoder.getSampleRate(): Int = ctlQuery(OPUS_GET_SAMPLE_RATE_REQUEST)
 ...
 */
fun OpusProjectionDecoder.getFinalRange(): Int = ctlQuery(OPUS_GET_FINAL_RANGE_REQUEST)
 ...
 */
fun OpusProjectionDecoder.getBandwidth(): Int = ctlQuery(OPUS_GET_BANDWIDTH_REQUEST)
 ...
 * @return [OPUS_OK] on success
 */
fun OpusProjectionDecoder.resetState(): Int = ctl(OPUS_RESET_STATE, 0)
 ...
 * @return [OPUS_OK] on success
 */
fun OpusProjectionDecoder.setIgnoreExtensions(ignore: Boolean): Int = ctl(OPUS_SET_IGNORE_EXTENSIONS_REQUEST, if (ignore) 1 else 0)

/**
 * Gets whether the decoder is ignoring extensions.
 * @return true = Ignoring extensions, false = Processing extensions
 */
fun OpusProjectionDecoder.getIgnoreExtensions(): Boolean = ctlQuery(OPUS_GET_IGNORE_EXTENSIONS_REQUEST) == 1

/**
 * Decodes a complete projection Opus packet into PCM.
 *
 ...
 */
fun OpusProjectionDecoder.decode(data: ByteArray, frameSize: Int): ShortArray {
...
/**
 * Decodes a complete projection Opus packet into floating point PCM.
 *
 * This is a simplified version that creates an output buffer of an appropriate size based on the frame size.
 * The decode_fec parameter is set to false, so this function won't perform packet loss concealment.
 *
 ...
 */
fun OpusProjectionDecoder.decodeFloat(data: ByteArray, frameSize: Int): FloatArray {
